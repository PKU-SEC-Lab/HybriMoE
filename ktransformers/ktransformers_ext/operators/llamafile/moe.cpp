...
void MOE::forward_many(int qlen, int k, const uint64_t* size_per_token, const uint64_t* expert_ids, const float* weights, const void* input, void* output, Backend* backend) {
    for (int i = 0; i < config_.expert_num; i++) {
        m_local_num_[i] = 0;
    }
...
void MOE::forward(int qlen, int k, const uint64_t* size_per_token, const uint64_t* expert_ids, const float* weights, const void* input, void* output, Backend* backend) {
    ...
    forward_many(forward_len, k, size_per_token, expert_ids, weights, input, output, backend);
